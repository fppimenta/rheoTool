...
{
    defineTypeNameAndDebug(MarkoSiggia, 0);
    addToRunTimeSelectionTable(springModel, MarkoSiggia, dictFS);
}
}
 
// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::springModels::MarkoSiggia::MarkoSiggia
(
...
MAXFRACL_(dict.subDict("springModelProperties").lookupOrDefault<scalar>("cutOffStretch", .99))   
{  
...
Foam::tmp<Foam::vectorField> Foam::springModels::MarkoSiggia::fSpringI
(
  vectorField& p, 
  label mi,
  bool hasDimensions,
...
void Foam::springModels::MarkoSiggia::fSIM
(
  label mi,
  label cmpi, 
...
)
{
 ...
 scalar f(2. * dt * D_[gI] * Nks_[gI] / (Ls_[gI] * Ls_[gI]) ); // dimless
 ...
 forAll(mSpr_[mi], bi)
  {
     ...
     F =  -f * ( lmrj - .25 + .25/Foam::pow(1.-lmrj, 2) ) * rj/mrj;  
...
void Foam::springModels::MarkoSiggia::jacobianSIM
(
  label mi,
  label cmpi,
  const Field<vector>& x,
...
{  
  ...
  forAll(mSpr_[mi], bi)
  {
     ...
     Rab =
        (f/lmab) 
      * ( 
           ( 1./(4.*Foam::pow(lmab-1., 2)) + lmab - 1./4. ) * (rab/(lmab*mab) - 1.)
         - (rab/mab) * (1. - 1./(2.*Foam::pow(lmab-1.,3)) ) 
        );
