...
{
    defineTypeNameAndDebug(CohenPade, 0);
    addToRunTimeSelectionTable(springModel, CohenPade, dictFS);
}
}
...
Foam::springModels::CohenPade::CohenPade
(
...
Foam::tmp<Foam::vectorField> Foam::springModels::CohenPade::fSpringI
(
  vectorField& p, 
  label mi,
  bool hasDimensions,
...
void Foam::springModels::CohenPade::fSIM
(
  label mi,
  label cmpi, 
...
)
{
 ...
 scalar f(dt * D_[gI] * Nks_[gI] / (Ls_[gI] * Ls_[gI]) ); // dimless
 ...
 forAll(mSpr_[mi], bi)
  {
     ...
     F =  -f * ( (3. - lmrj*lmrj)/(1. - lmrj*lmrj) ) * rj;   
...
void Foam::springModels::CohenPade::jacobianSIM
(
  label mi,
  label cmpi,
  const Field<vector>& x,
...
{  
  ...
  forAll(mSpr_[mi], bi)
  {
     ...
     Rab =
        f 
      * ( 
             ( 2.*rab/(lmab*lmab - 1.) ) * ( (lmab*lmab - 3.)/(lmab*lmab - 1.) - 1.  )
          -  (lmab*lmab - 3.)/(lmab*lmab - 1.) 
        );
