 ...
 solveAlone_(false)
{
 ...
 if (solveAlone_)
 {
   cps_.reset
   (
     new coupledSolver("ciPsi",phi.mesh().name(),phi.mesh().time(),phi.mesh().solutionDict())
   );
   
   // Add mesh
   cps_->insertMesh(phi.mesh());
 ...
 else
 {
   ...
   (
     phi.time().lookupObject<coupledSolver>(word("Uptau."+phi.mesh().name()))
   );
...
const
{
 ...
 {
   coupledSolver& cps = psi_.time().lookupObjectRef<coupledSolver>(word("Uptau."+psi_.mesh().name()));
...
void Foam::EDFEquations::NernstPlanckCoupled::solveWithUptau()
{
 ...
 coupledSolver& cps = phi().time().lookupObjectRef<coupledSolver>(word("Uptau."+phi().mesh().name()));
