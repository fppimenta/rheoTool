...
:
    ...
    ItensorCorr("Identity", dimless, symm(tensor::I)),
    ...
    funcPTT(0)    
{
   ...
   // Adjust ItensorCorr
    ItensorCorr.value().xx() = dims_.x();
    ItensorCorr.value().yy() = dims_.y();
    ItensorCorr.value().zz() = dims_.z();
...
)
{
    ...
    volScalarField tauDMag(Foam::mag(tau_-ItensorCorr*tr(tau_)/nDims)/Foam::sqrt(2.));
