...
// Change 2022: replaced all INSERT_VALUES by ADD_VALUES due to run-time error
// originated by the mixed use of both without intermediate call to 
// MatAssemblyBegin(A,MAT_FLUSH_ASSEMBLY).
// Reasoning: when cmpV = 0 (updateA = true or first component of a field) A and b   
// are set in a just pre-allocated matrix, so we add to 0 in each coefficient. When
// cmpV != 0, we always reset the coefficients of A and b at the beginning, so we still
// keep adding to 0.  
...
)
{
 ...
 if (cmpV == 0)
 {
  ...
  // Optional (but never call before pre-allocation):
 ...
 if (eqn.symmetric())
 {
   ...
   { 
     ...
     // ij Off-diagonal
     ierr = MatSetValues(A,1,&row,1,&col,&eqn.upper()[face],ADD_VALUES);CHKERRV(ierr);
     // ji Off-diagonal
     ierr = MatSetValues(A,1,&col,1,&row,&eqn.upper()[face],ADD_VALUES);CHKERRV(ierr);   
   }
 }
 else
 {
   for (label face=0; face<nFaces; face++)
   {
     ...
     // ij Off-diagonal
     ierr = MatSetValues(A,1,&row,1,&col,&eqn.lower()[face],ADD_VALUES);CHKERRV(ierr);
     // ji Off-diagonal
     ierr = MatSetValues(A,1,&col,1,&row,&eqn.upper()[face],ADD_VALUES);CHKERRV(ierr);   
   }
 }
 ...
 {  
   // Diagonal
   row = ilower + cellI;  
   ierr = MatSetValues(A,1,&row,1,&row,&eqn.diag()[cellI],ADD_VALUES);CHKERRV(ierr);
