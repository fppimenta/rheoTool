...
void Foam::extSTFvPatchSymmTensorField::updateCoeffs()
{
    if (updated())
    {
     ...
     word sTensorFName( this->dimensionedInternalField().name() );
     ...
     for (int i=0; i<6 ;i++) 
     {
        ...
        tmp< GeometricField< vector, fvPatchField, volMesh> > tgradT= fvc::grad(taufield.component(i), "extSTGrad");

        const GeometricField< vector, fvPatchField, volMesh>& gradT = tgradT();
