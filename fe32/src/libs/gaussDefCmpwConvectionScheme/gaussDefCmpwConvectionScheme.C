...
) const
{
    ...
    const unallocLabelList& neig = mesh.neighbour();
        ...
        forAll(vf.boundaryField(), patchI)
         {
           ...
           if (vf.boundaryField()[patchI].coupled())
             {               
              ...
              const fvsPatchScalarField& patchPhi = faceFlux.boundaryField()[patchI];
              const unallocLabelList&  fC= mesh.boundaryMesh()[patchI].faceCells();
...
) const
{
    ...
    scalarField alphaL(10, 1.0); scalarField betaL(10, 0.); scalarField bL(10, 0.); // Slightly oversized
    ...
    for (int cmp=0; cmp<Type::nComponents; cmp++)
     {
       ...
       tmp< GeometricField< vector, fvPatchField, volMesh> > gradcmpT= fvc::grad(vf.component(cmp));
       const GeometricField< vector, fvPatchField, volMesh>& gradcmp = gradcmpT();
           ...
           if (vf.boundaryField()[patchI].coupled())
             {
              ...
              const vectorField& deltasP = sfp.patch().delta(); 
...
{    
    ...
    tmp< GeometricField< vector, fvPatchField, volMesh> > gradcmpT= fvc::grad(vf);
    const GeometricField< vector, fvPatchField, volMesh>& gradcmp = gradcmpT();
