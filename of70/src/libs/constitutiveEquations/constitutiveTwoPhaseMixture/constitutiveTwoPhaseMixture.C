...
Foam::constitutiveTwoPhaseMixture::divTauMF
(
  volVectorField& U
)
{
...
Foam::tmp<Foam::fvVectorMatrix>
Foam::constitutiveTwoPhaseMixture::divTauMFThermo
(
  volVectorField& U
)
{
   ...
   return 
   (
      phase1_->divTauSThermo(U, bAlpha1) 
    + phase2_->divTauSThermo(U, bAlpha2)
    + fvc::div(tauMF_,"div(Sum(tau))")          
   );         
}
