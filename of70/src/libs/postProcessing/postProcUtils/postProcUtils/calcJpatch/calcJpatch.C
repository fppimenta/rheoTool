...
patchL_(dict.lookup("ListOfPatches"))
{
   ...
   //- Read fluid properties      
    const dictionary& elecDict = mesh.lookupObject<IOdictionary>("electricProperties");
    ...
    {  
       ...
       dimensionedScalar dD_ = specEntries_[specI].dict().lookup("D"); D_.append(dD_.value());
       dimensionedScalar dz_ = specEntries_[specI].dict().lookup("z"); z_.append(dz_.value());
       specL_.append(specEntries_[specI].keyword());
    }
...
//****  User-defined function STARTS here *****//
    ...
    forAll(patchL_, pi)
    {
      ...
      label pI(mesh().boundaryMesh().findPatchID(patchL_[pi]));
      ...
      forAll(specL_, si)
        {           
          ...
          const volScalarField& specI = mesh().lookupObject<volScalarField>(specL_[si]);
