...
:
    fixedValueFvPatchVectorField(p, iF),
    elecM_(readScalar(dict.lookup("elecMobility0"))),
    sigma0_(readScalar(dict.lookup("sigma0"))),
...
void Foam::slipSigmaDependentFvPatchVectorField::updateCoeffs()
{
    ...
    vectorField::operator=( elecM_ * Ef.boundaryField()[patch().index()] * Foam::pow(sigma_.boundaryField()[patch().index()]/sigma0_, m_));
...
void Foam::slipSigmaDependentFvPatchVectorField::write(Ostream& os) const
{
    ...
    os.writeKeyword("elecMobility0")
        << elecM_ << token::END_STATEMENT << nl;
    os.writeKeyword("sigma0")
        << sigma0_ << token::END_STATEMENT << nl;
    os.writeKeyword("m")
        << m_ << token::END_STATEMENT << nl;
    writeEntry("value", os);
}    
