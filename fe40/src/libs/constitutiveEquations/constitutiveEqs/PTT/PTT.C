...
:
    ...
    alpha_("alpha", dimensionSet(0, 0, 0, 0, 0, 0, 0), 1.0),
    beta_("beta", dimensionSet(0, 0, 0, 0, 0, 0, 0), 1.0),
    ...
    PTTFunction_(PTTFunctionNames_.read(dict.lookup("destructionFunctionType")))
{
 ...
 {
    dimensionedScalar talpha_(dict.lookup("alpha")); alpha_.value() = talpha_.value();
    dimensionedScalar tbeta_(dict.lookup("beta")); beta_.value() = tbeta_.value();
    ...
    if (alpha_.value()<=0 || beta_.value()<=0)
    {
      ...
      ) << "Both alpha and beta should be positive values "
        << "for the Mittag-Leffler function to converge." << endl << exit(FatalError);
    }
...
void Foam::constitutiveEqs::PTT::correct()
{
   // Velocity gradient tensor
    ...
    {    
      case pfLinear :      
        tauEqn += fvm::Sp(epsilon_/etaP_*tr(tau_) + 1./lambda_, tau_);
        break;
      
      case pfExpt : 
        tauEqn += fvm::Sp( (1./lambda_)*Foam::exp(epsilon_*lambda_/etaP_*tr(tau_)), tau_);
        break;
      
      case pfGen :
        scalar gammaBeta(gammaFunValues_[0]);
        volScalarField z = epsilon_*lambda_*tr(tau_)/etaP_;
        ...
        forAll(z, i)
        {
         ...
         if (k == MLmaxIter_)    
         {
           WarningIn("void Foam::constitutiveEqs::PTTLog::correct()")
        ...
        tauEqn += fvm::Sp( (1./lambda_)*gammaBeta*Eab, tau_); 
        break;   
    }
