...
namespace Foam
{
    defineTypeNameAndDebug(MNernstPlanck, 0);
    addToRunTimeSelectionTable(EDFEquation, MNernstPlanck, dictionary);
}

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::MNPSpecie::MNPSpecie
(
    const word& name,
...
Foam::MNernstPlanck::MNernstPlanck
(
    const word& name,
    const surfaceScalarField& phi,
    const dictionary& dict
...
Foam::tmp<Foam::volVectorField> Foam::MNernstPlanck::Fe() const
{    
...
void Foam::MNernstPlanck::correct()
{
   ...
   //- Equation for the intrinsic potential
    ...
    //- Equation for each ionic specie 
        ...
        volScalarField cT(species_[0].ci());
        volScalarField cD(1. - species_[0].ci()*species_[0].ai()*species_[0].ai()*species_[0].ai()*AK_);
        
        forAll (species_, i)
         {
            cT += species_[i].ci();
            cD -= species_[i].ci()*AK_*species_[i].ai()*species_[i].ai()*species_[i].ai();
        ...
        forAll (species_, i)
         {
           ...
           dimensionedScalar ai(species_[i].ai());
		 ...
		 fvScalarMatrix ciEqn 
		  ...
		  ==  
		    fvm::laplacian(species_[i].Di(), ci, "laplacian(D,ci)")   	 
                  ...
                  + ai*ai*ai*fvc::laplacian(species_[i].Di()*ci*AK_/cD, cT, "laplacian(gamma,cT)")
		 	    
		  );        
		
		  ciEqn.relax(phi().mesh().solutionDict().equationRelaxationFactor("ci"));
		  res=ciEqn.solve(phi().mesh().solutionDict().solver("ci")).initialResidual();
