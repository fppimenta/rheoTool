...
#include "boltzmannEquilibriumLogFvPatchScalarField.H"
...
Foam::boltzmannEquilibriumLogFvPatchScalarField::
...
boltzmannEquilibriumLogFvPatchScalarField
(
    const boltzmannEquilibriumLogFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
:
    ...
    zib_(ptf.zib_),
    c0_(ptf.c0_().clone().ptr()),
    psi0_(ptf.psi0_().clone().ptr())
{}
...
:
    fixedValueFvPatchScalarField(p, iF),
    zib_(0),
    c0_(DataEntry<scalar>::New("cLog0", dict)),
    psi0_(DataEntry<scalar>::New("psi0", dict))
{
...
boltzmannEquilibriumLogFvPatchScalarField
(
    const boltzmannEquilibriumLogFvPatchScalarField& tppsf,
    const DimensionedField<scalar, volMesh>& iF
)
:
    fixedValueFvPatchScalarField(tppsf, iF),
    zib_(tppsf.zib_),
    c0_(tppsf.c0_().clone().ptr()),
    psi0_(tppsf.psi0_().clone().ptr())
{}


// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

void Foam::boltzmannEquilibriumLogFvPatchScalarField::updateCoeffs()
{
    if (updated())
    {
        return;
    ...
    scalarField::operator=( c0 - alpha*(psib_ - psi0) );
     
    fixedValueFvPatchScalarField::updateCoeffs();
}

 
void Foam::boltzmannEquilibriumLogFvPatchScalarField::write(Ostream& os) const
{
