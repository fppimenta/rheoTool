...
bool Foam::NernstPlanckLog::checkForPhiE
(
 const word& name,
...
:  
    ...
    l0_(logCi_.internalField()),
    l00_(logCi_.internalField()),
    zi_(dict.lookup("z")),
    Di_(dict.lookup("D")),
    namei_(name)  
{}
    ...
    if (solvePhiE_)
     {
       if (psiContrib_)
        ...
        /*
          ...
          const polyPatch& pp(phi().mesh().boundaryMesh()[4]);
           
          forAll( rhoE.boundaryField()[4], fi)
           {                       
              
             label  cell1=pp.faceCells()[fi];
 
             label oppFacei = phi().mesh().cells()[cell1].opposingFaceLabel(fi+pp.start(),phi().mesh().faces()); //ID face oposta

             label  cell2 = phi().mesh().owner()[oppFacei]; //ID cell 2         

             if (cell2 == cell1)
             {
	       cell2 = phi().mesh().neighbour()[oppFacei];
             }
...
for (int k=0; k<1; k++)
 ...
 /*          
           scalarField& lci0(species_[i].l0());
           scalarField& lci00(species_[i].l00());
           
           lci00 = lci0;
           lci0 = lci.internalField();
           
           scalarField ci0(Foam::exp(lci0));
           scalarField ci00(Foam::exp(lci00));
...
*/            
	...
	//	  + fvc::laplacian(ca*cif, phiE_+psi_, "laplacian(EMigD,phiE)")
		  + fvc::div((eMigFluxp*ca - phi()) * cif)
		    
		  );        
		
		//Info << max(ci0) << tab << min(ci0) << tab << max(ci00) << tab << min(ci00) << nl << endl;
		 
	//	ciLogEqn += timeDer(ci, ci0, ci00, lci, lci0, lci00 ) ;	
	//	ciLogEqn += fvm::ddt(ci*icUnit, lci);
		
		  ciLogEqn.relax(phi().mesh().equationRelaxationFactor("ciLog"));
		  res=ciLogEqn.solve(phi().mesh().solver("ciLog")).initialResidual();
...
Foam::tmp<Foam::fvMatrix<Foam::scalar> > Foam::NernstPlanckLog::timeDer
( 
  scalarField& rho,
  scalarField& rho0,
  scalarField& rho00, 
...
)
{
    ...
    tmp<fvMatrix<scalar> > tfvmv
    (
        new fvMatrix<scalar>
        (
            a,
            a.dimensions()*dimVol/dimTime
        )
    );

    fvMatrix<scalar>& fvmav = tfvmv();
    
    fvmav.diag() =  1.5 * phi().mesh().V() * rho * (1./dt_);

    fvmav.source() = phi().mesh().V() * (1./dt_) * ( 2. * a0 * rho0 - 0.5 * a00 * rho00 );
