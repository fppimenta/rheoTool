...
namespace Foam
{
    defineTypeNameAndDebug(sPTTexpLog, 0);
    addToRunTimeSelectionTable(constitutiveEq, sPTTexpLog, dictionary);
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::sPTTexpLog::sPTTexpLog
(
    const word& name,
...
void Foam::sPTTexpLog::correct()
{
    ...
    (
       ...
       - (theta_&omega)
       + 2.0 * B
       + (1.0/lambda_) * Foam::exp( epsilon_*( tr( (eigVecs_ & eigVals_ & eigVecs_.T()) ) - 3.) )
        * (
             eigVecs_ &
               (
                  inv(eigVals_)  
