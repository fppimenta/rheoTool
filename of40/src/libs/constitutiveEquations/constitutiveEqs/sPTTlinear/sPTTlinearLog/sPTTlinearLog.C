...
namespace Foam
{
    defineTypeNameAndDebug(sPTTlinearLog, 0);
    addToRunTimeSelectionTable(constitutiveEq, sPTTlinearLog, dictionary);
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::sPTTlinearLog::sPTTlinearLog
(
    const word& name,
...
void Foam::sPTTlinearLog::correct()
{
   // Decompose grad(U).T()
    ...
    (
       ...
       - (theta_&omega)
       + 2.0 * B
       + (1.0/lambda_) * ( 1. + epsilon_*( tr( (eigVecs_ & eigVals_ & eigVecs_.T()) ) -3.) )
        * (
             eigVecs_ &
               (
                  inv(eigVals_)  
